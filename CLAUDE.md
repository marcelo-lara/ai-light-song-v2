...
| `mcp/` server + delivery surface | **built and green.** Three tools (`list_songs`, `get_song_overview`, `get_detail`); all nine top-level files per song (`info`, `beats`, `hints`, `sections`, `song_event_timeline`, `genre`, `drum_events`, `loudness`, `arrangement_state`) are **hard-required** — no degraded mode, a missing one errors naming it — each carrying a `field_sources` attribution header. `get_detail`'s structural view carries a `beats` block (`time`/`bar`/`beat`/`downbeat_confidence`, undecimated, past the 5 s dense cap). `get_song_overview` carries `review_warning` when any section has a `seed_unreviewed` field |
