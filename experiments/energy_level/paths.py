...
`sections.json`'s `energy` field (docs/product-refinement-v3.6.md item 5/6).
Reads only top-level published `data/analysis/{song}/*.json`
(`loudness.json`, `arrangement_state.json`) plus segment spans from
...
REPO_ROOT = Path(
    os.environ.get("ENERGY_LEVEL_EXP_REPO", Path(__file__).resolve().parents[2])
)
...
def proposals_path(song: str) -> Path:
    return song_dir(song) / "reference" / "proposals" / "energy_level.json"
