...
#: present 40.8% of `ayuni`). Matches items 3-5's scoring corpus.
...
def mix_audio_path(song: str) -> Path:
    """The "mix" channel. There is no `stems/mix.wav` — `SongPaths` (see
    `src/analyzer/paths.py`) writes only `vocals`/`bass`/`drums`/`harmonic`
    stems, no re-mixed output. "The mix" is the original song audio the
    pipeline was given, `data/songs/<song>.mp3` — matches
    `experiments/clap/paths.py::audio_path`'s own convention for "the whole
    song, unstemmed"."""
...
def proposal_path(song: str) -> Path:
    return ANALYSIS_ROOT / song / "reference" / "proposals" / "svd_tagger.json"
