...
it has no dependency on `experiments/clap/cache/` being populated.
"""
...
REPO_ROOT = Path(os.environ.get("CLAP_VOICENESS_EXP_REPO", Path(__file__).resolve().parents[2]))
...
#: present 40.8% of the song). Matches items 3-4's scoring corpus.
SCORING_CORPUS = GOLD_SONGS + ["ayuni"]


def all_analysed_songs() -> list[str]:
    """Every song that has been through the pipeline at all (has a stems
    dir), for `--all` — not restricted to the scoring corpus."""
    return sorted(
        p.name
...
def proposal_path(song: str) -> Path:
    return ANALYSIS_ROOT / song / "reference" / "proposals" / "clap_voiceness.json"
