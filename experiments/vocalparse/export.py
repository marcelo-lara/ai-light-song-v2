...
NOTE = (
    ...
    "`alignment: span` or `unavailable` has no trustworthy per-line timing."
)
...
def export_song(song: str) -> Path:
    ...
    payload = _merge(song, [whisper, vp_source])
    path = proposal_path(song)
...
def summary_line(song: str, path: Path) -> str:
    ...
    if src is None:
        return f"  {song:<40} (no vocalparse source)"
    return (f"  {song:<40} {len(src['lines']):3d} lines   align={src['alignment']:<11} "
            f"bpm={src.get('bpm')}")
