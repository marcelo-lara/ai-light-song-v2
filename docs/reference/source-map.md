...
| 3 | `section_clues.py` | `section-clues` (v3.6 item 10) — re-fuses `energy`, `energy_confidence`, `tension`, `tension_confidence`, `rhythm` onto the published `sections.json`, precedence `human` > best ported producer (`energy_level`/`tension_shape`/`rhythm_drum_ioi`/`rhythm_stem_autocorr`/`rhythm_vocal_onsets`) > `seed_unreviewed`. Reads `beats.json`, `drum_events.json`, `loudness.json`, `arrangement_state.json`, `song_event_timeline.json`, `reference/human/{segments.json,segments.seed.json}` and `artifacts/whisperx-vad/vocal_onsets.json` (raises if that last one is missing). Never audio. Runs after `contest-section-function` |
