...
| `song_event_timeline.json` | `type`, `start_time`, `end_time`, `confidence`, `intensity`, `gesture_id` → `gestures`; `section_id` → `sections` (v3.7 item 6 — attributed by timestamp against the published `sections.json`, never `artifacts/section_segmentation/sections.json`; `build-gestures` now runs after `build-ui-data` for this reason) |
