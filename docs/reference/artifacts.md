...
| `human/segments.json` | hand-marked gold section boundaries: a flat `[{ start, end, label }]` list, `label` in `docs/segments-vocabulary.md` terms (v3.5 item 10) | validation (`human_segments` boundary recall/precision/F1 in the phase-1 report) **and** publish fusion — `ui_data.build_ui_data` rebuilds `sections.json` from these spans outright when present for a song, at fixed `confidence: 0.8`, `function_confidence`/`function_status`/`same_label_as` still inherited from the best-overlapping allin1 section. Present today for `_test_song`, `ayuni`, `"What a Feeling - Courtney Storm"`. Never read by an interpret/relate stage |
