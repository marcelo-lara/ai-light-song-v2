...
| `section_segmentation/sections_display.json` | **New in v3.6 item 8.** `{ schema_version, song_name, generated_from, sections[] }`; each row `{ section_id, label, description, chord_progression }`, joined to `sections.json` by `section_id` | the display text dropped from top-level `sections.json` (`label` — the confidence-folded display string; `description` — a restated function+ordinal sentence; `chord_progression` — the section's dominant repeating chord sequence). Written by the publish stage (`ui_data.py`'s `build_ui_data`). **Not MCP-exposed** — read by the debugger UI only |
...
| `layer_a_harmonic.json` | `global_key`, `chords[]` (`time`, `end_s`, `chord`, `confidence`) | chord-change timing and tonal identity. Since v3.6 item 8 dropped `beats.json`'s per-beat `chord` field at top level, this is the only place to read chord labels — time-indexed, not beat-indexed. **Not MCP-exposed** |
...
| `human/segments.json` | hand-marked gold section boundaries: a flat `[{ start, end, label }]` list, `label` in `docs/segments-vocabulary.md` terms (v3.5) | validation (`human_segments` boundary recall/precision/F1 in the phase-1 report) **and** publish fusion — `ui_data.build_ui_data` rebuilds `sections.json` from these spans outright when present for a song, at fixed `confidence: 0.8`, `function_confidence`/`function_status`/`same_label_as` still inherited from the best-overlapping allin1 section. Full precedence in [`analysis.segments.md`](analysis.segments.md). Never read by an interpret/relate stage |
