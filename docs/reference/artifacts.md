...
| `sections.json` | `section_id`, `start`, `end`, `label`, `description`, `function`, `confidence` → `human` if `reference/human/segments.json` exists for the song, else `moises` if `reference/moises/segments.json` exists, else `allin1` — full precedence and rationale in [`analysis.segments.md`](analysis.segments.md). `function_confidence`, `function_status`, `same_label_as` → always `allin1`; `key`, `chord_progression` → `harmonic`. On a song with a contested row: `function_status` → `section_function` and `contested_by` → `section_function` (present only on contested rows); byte-identical header otherwise |
