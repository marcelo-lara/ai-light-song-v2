...
layer files (`layer_a_harmonic`, `layer_c_energy`) and the `validation/`
...
Generated by the phase-3 `gestures` stage (plan v3.0 item 9; replaces the
whole Epic-5 `event_*` chain, measured at chance against the gold set).
`events[]`, each a **flat** row (never a composite with nested `phases[]`):
`type`, `start_time`, `end_time`, `confidence`, `intensity`, `section_id`,
`section_name`, `provenance`, `summary`, `evidence_summary`.

- `type` is either a gesture-phase name (`approach`, `build`, `tension`,
  `impact`, `release` — the named internal phases of one composite gesture
  anchored on a detected impact) or a section-pair transition of the form
  `"<from_label> → <to_label>"`. **A drop is never named directly** — the
  vocabulary can only say "a build of this shape happens here," never "this
  is the drop" (constitution §5.2). A phase absent from a gesture (e.g. no
  `tension` row) means no supporting sound-design primitive was found for
  it — never guessed (constitution §2).
- Unscoped, the MCP server projects **only** `type`, `start_time`,
  `end_time`, `intensity`, `section_id` (a table of contents). The prose
  `summary` / `evidence_summary` are delivered **only** when a section pass
  asks for its own section.
- So: make `type`, the time window, and `intensity` (0–1) precise — those are
  ...
  actionable for the section pass — a musical description of the moment
  ("a rising riser builds into an impact here"), never an internal
  implementation note.
- Every gesture-phase event already carries the primitive detector's own
  `evidence_summary` (e.g. `"high-band r2=0.82 over 4 bars, delta=0.31x
  range"`); every transition event carries the section-segmentation
  boundary's own confidence and labels. There is no separate machine-events
...
2. **`song_event_timeline.json`:** a lean set of well-timed discrete gesture
   phases and section transitions, with `intensity`, honest `confidence`,
   `section_id`, and an actionable `summary`.
...
- The intermediate layer files, pattern mining, and `validation/` reports —
  not on the MCP surface. Useful only as upstream inputs to the seven
  top-level files.
