> **ARCHIVED — closed issues, kept for their evidence.**
> This is the resolved half of the analysis issue tracker. Open issues live in
> [`../issues.md`](../issues.md), which by rule contains **only** pending ones
> (constitution §4.2). Nothing here is a specification, and a `solved` status
> here is a claim about that issue's own success condition, not about the
> current quality of the stage it touched.

# Analysis issues — solved

Moved out of `docs/issues.md` on 2026-09-02 when the "pending only" rule was
adopted. Every issue below was closed against its own stated success condition.

## Read ISS-008 / ISS-009 / ISS-010 with care

Those three closed narrow, per-song success conditions about section-boundary
placement (a zero-start, a 19.0 s onset, three micro-structure windows in one
track). They were satisfied as written. They are **not** evidence that section
segmentation works: the 2026-09 measurement in
[`../../experiments/drop_detection/README.md`](../../experiments/drop_detection/README.md)
puts the shipped `sections.json` at **0/7 boundaries within +/-1.0 s** of a
human-marked impact, below a librosa baseline. A passing narrow gate on one song
is compatible with a stage that is broadly wrong — which is the more useful
lesson here than any individual fix.
...
- Notes:
  ...
  - Implemented: inferred chords are validated first, explicit mismatch attribution is written to the report, failing inferred harmonic output is preserved under `data/analysis/<Song - Artist>/artifacts/harmonic_inference/layer_a_harmonic.inferred.json`, and the canonical harmonic layer is promoted from the Moises reference for downstream phases when the stricter gate fails.
...
- Notes:
  ...
  - Current progress: section labels now use a more contextual vocabulary (`momentum_lift`, `groove_plateau`, `contrast_bridge`, `focal_lift`, `flowing_plateau`, `breath_space`) and `data/analysis/<Song - Artist>/sections.json` now carries non-empty descriptions derived from those labels.
