> **ARCHIVED — a parallel plan track, which the constitution now forbids.**
> Constitution §4.1: one `implementation-plan-*.md` for the whole release, in
> `docs/`, never one per component. Nothing here was built. Fold any item you
> still want into the current `docs/product-refinement-vX.Y.md` rather than
> implementing from this file. The component itself is still defined in
> [`../reference/mcp-server-product-definition.md`](../reference/mcp-server-product-definition.md).

# Implementation Plan — MCP Server v1

Turns [product-definition.md](../reference/mcp-server-product-definition.md) into ordered, validated
...
[product-definition.md](../reference/mcp-server-product-definition.md) §7 and §11.
